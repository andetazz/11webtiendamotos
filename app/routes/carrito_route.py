...
def obtener_carrito_agrupado():
    ...
    - idprod, nompro, precio, cantidad
    - subtotal base
    - descuento_aplicado
    - iva_aplicado
    - total_final (con descuento + IVA)
    También devuelve el total general acumulado.
    """
    data = db.session.query(
        Carrito.idproducto.label('idprod'),
        Productos.img1.label('img1'),
        Productos.nombre.label('nompro'),
    ...
    for row in data:
        ...
        iva = Decimal(str(row.iva))

        subtotal = precio * cantidad
        descuento_aplicado = descuento  # ya viene en valor monetario
        subtotal_descuento = subtotal - descuento_aplicado
        iva_aplicado = subtotal_descuento * (iva / Decimal('100'))
        total_final = subtotal_descuento + iva_aplicado

        item = {
            "idprod": row.idprod,
            "img1": row.img1 if row.img1 else "productos.png",
            "nompro": row.nompro,
            "precio": float(precio),
            ...
            "iva_aplicado": float(iva_aplicado),
    ...
    return resultado, float(total_general)

@bp.route('/carrito/listtodos')
def listarcarritotodos():
   
    resultado = obtener_carrito_agrupado()
    carrito_resumen = resultado[0]
    total_general = resultado[1]

    return render_template('carrito/listar_carrito.html', data=carrito_resumen,total_general=total_general)
...
def vercompras():
    dataventas_t = Ventas_t.query.filter_by(iduser= current_user.iduser).order_by(desc(Ventas_t.idventa)).all()
...
def listar_compras():
    dataventas_t = Ventas_t.query.order_by(Ventas_t.idventa).all()

    return render_template('carrito/listar_compras.html', data=dataventas_t) 

@bp.route('/carrito/addestadocompra')
def addestadocompra():
